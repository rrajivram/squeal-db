...
**Keys** are `DBIdType`: `Int(u64)` or `Rec(IndexKey)` for composite/multi-column keys (`IndexKey` wraps a `Vec<ValueItem>` — integers, doubles, strings, blobs, datetimes, booleans, or `Null`; a plain string id is just a single-field `Rec`, via `DBIdType::from(String)` — there's no separate byte-vector id kind). `Int` orders by a hash of its value (`DBIdType::hashed()`), not structurally — this matches the B+Tree's own iteration/storage order but means two *different* `Int` ids can compare `Equal` under `Ord` (a hash collision) while still being `!=` under `PartialEq`. `Rec` orders structurally (field-by-field), which is the one case range scans over multiple columns actually make sense.
...
**Overflow pages**: a single tuple too large to fit on an *empty* page (`Page::can_store`'s "empty page always accepts" exception) spills across a chain of overflow pages, up to `MAX_OVERFLOW_PAGES = 1024`. Exceeding that cap returns a generic `StoreError::UnknownError`, not `TupleTooLarge` — a caller does hit `TupleTooLarge` well before this point in the ordinary case (see below), but this specific "even the overflow chain isn't enough" edge still isn't labeled distinctly. This overflow mechanism is *only* valid for that single-oversized-tuple case; a multi-tuple data page must never be routed through it (a real bug where `update()` could push a fully-packed page over capacity and trigger this path was found and fixed this session — see `bplustree.rs::update`'s and `buffer.rs::handle_large_page_size`'s own comments for the full story, plus the regression test `db.rs::test_table_scan_correct_after_updating_every_row_across_multiple_data_pages`).

**`StoreError::TupleTooLarge`** is very much wired up, on two related but distinct paths: `FixedTuplePage::add`/`replace` (an index page's fixed per-entry budget — see `BPlusTree::new`'s `index_entry_size` below) and `ValueItem::validate` (a `Str`/`Blob` field exceeding its own declared reserved capacity, checked by `IndexKey::new_from`). `Page::add_tuple` checks the fixed-per-entry budget (via `Page::record_size`) *before* its generic aggregate-bytes `can_store` check, specifically so a tuple that structurally can never fit a page's fixed record size is reported as `TupleTooLarge` rather than a generic, less-actionable `PageCapacityError` — the latter would otherwise mask the real cause once the page already holds other entries (fixed this session; see `page.rs`'s and `bplustree.rs`'s own comments on `add_tuple`/`insert_recursive`'s leaf branch, and the regression tests around them).
...
- **`IndexKey`/`ValueItem`'s reserved-capacity `u32`** is validated against its own field now, but `size()`/`Eq`/`Ord`/`hash()` still disagree with each other about whether it's part of a value's identity — see its own section above. Validating against a *table's* index-entry cap is now possible too (`index_entry_size` is an explicit, caller-chosen parameter rather than a hardcoded 64 bytes — see "Sizing the index for your key shape" above), but nothing computes that bound for you; you still have to work it out from your own key shape.
...
- **An oversized tuple that exceeds the overflow-chain cap** (`MAX_OVERFLOW_PAGES = 1024`, roughly `1024 × page_size` bytes) surfaces as a generic `UnknownError`, not a distinctly-labeled one — see the overflow-pages note above. In the ordinary case (an index entry over its table's `index_entry_size`, or a `Str`/`Blob` field over its own declared capacity) you do get a clear `TupleTooLarge`; this is only about that much narrower, much larger edge case.
- **`#![allow(dead_code)]` is set crate-wide** in `lib.rs`, which is part of why `TableType::Index` exists without so much as a compiler warning flagging it as unused. If you're extending this codebase, don't assume "it compiles cleanly" means "everything defined is actually wired up."
...
- ~~`index_entry_size` hardcoded to `MAX_ENTRY_BYTES = 64` inside `BPlusTree::new`~~ — now an explicit parameter callers must size deliberately (`Db::create_table_with_index_entry_size`). See "Sizing the index for your key shape" above.
- ~~An oversized index entry on an already-populated leaf page could hit a hard panic or a generic `PageCapacityError` instead of `TupleTooLarge`~~ — `can_store`'s aggregate-bytes check could return false for the same underlying reason as the tuple's own fixed-record budget, masking the real cause (or, in `insert_recursive`'s leaf branch specifically, hitting a panic meant for a genuine invariant violation). Fixed by checking the per-entry budget first in both places. See the `TupleTooLarge` note above.
- ~~`DBIdType::Vec(Vec<u8>)` as a separate id kind~~ — removed; a plain string id (`DBIdType::from(String)`) is now a single-field `Rec(IndexKey)`, since `Rec` already covers that shape.
