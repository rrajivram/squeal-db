...
**Keys** are `DBIdType`: `Int(u64)` or `Rec(IndexKey)` for composite/multi-column keys (`IndexKey` wraps a `Vec<ValueItem>` — integers, doubles, strings, blobs, datetimes, or `Null`; a plain string id is just a single-field `Rec`, via `DBIdType::from(String)` — there's no separate byte-vector id kind). `Int` orders by a hash of its value (`DBIdType::hashed()`), not structurally — this matches the B+Tree's own iteration/storage order but means two *different* `Int` ids can compare `Equal` under `Ord` (a hash collision) while still being `!=` under `PartialEq`. `Rec` orders structurally (field-by-field), which is the one case range scans over multiple columns actually make sense.
