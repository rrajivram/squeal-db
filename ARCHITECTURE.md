...
`ValueItem::Str((String, u32))` and `ValueItem::Blob((Arc<[u8]>, u32))` carry a `u32` alongside the content — a **declared, reserved on-disk capacity** (`to_bytes` pads short content out to this length; presumably intended to support fixed-layout, in-place-updatable fields). In practice this is inconsistently enforced and, as you noted, effectively ignored:

- `size()` is driven entirely by the *reserved* capacity, not the actual content length — a field whose real content exceeds its declared capacity **under-reports its own size** (`to_bytes` still writes the full content regardless, so the byte count on disk and the accounted `size()` can genuinely disagree).
...
- **Nothing validates upfront** that content actually fits the declared capacity, or that a constructed `IndexKey` stays within `MAX_ENTRY_BYTES = 64` (the hard cap on a single index entry) before you try to insert it — you find out via a `PageCapacityError` at insert time, not a clear "this key is too big" error.

Net effect: if you build composite keys with `Str`/`Blob` fields, the reserved-capacity value is closer to a hint that mostly gets ignored than an enforced contract. Treat it as unreliable — don't rely on it for correctness, and keep composite keys well under the 64-byte index-entry cap.
